...
class Road:
    def __init__(self, x, y,
        ...
        self.car_sprites = [
            pygame.transform.scale(i, (self.car_width, self.car_height))
            for i in car_sprites]

        self.cars = []

        self.lines_x = [
            (i * self.width // 4 - self.width // 8) - self.car_width // 2
            for i in range(1, 5)]

        self.person = Person(self.width // 2,
                             self.height // 2,
                             self.width, self.height,
                             random.choice(self.car_sprites),
                             self.rocket_spawn)

        self.rocket_sprite = pygame.transform.scale(rocket_sprite,
                                                    (self.height // 50,
                                                     self.height // 10))
            ...
            while True:
                ...
                if not self.check_car_collision(
                        pygame.rect.Rect(
                            self.lines_x[line],
                            y, self.car_width,
                            self.car_height + self.car_height // 2)):
                    self.cars.append(
                        Car(
                            self.lines_x[line], y,
                            self.height, self.speed,
    ...
    def rocket_spawn(self):
        if self.rocket_count > 0:
            ...
            self.rockets.append(
                Rocket(
                    rocket_x, rocket_y,
                    self.height, self.rocket_sprite,
                    self.person.speed + self.width // 8))

    def move_rockets(self, time):
        for i in range(len(self.rockets)):
            if i < len(self.rockets) and self.rockets[i].move(time):
                del self.rockets[i]
    ...
    def move_cars(self, time):
        for i in range(len(self.cars)):
            if i < len(self.cars) and self.cars[i].move(time):
                del self.cars[i]
    ...
    def draw_interface(self):
        ...
        for i in range(self.rocket_count):
            self.surface.blit(
                self.rocket_sprite,
                (x + margin * i + self.rocket_sprite.get_width() * i,
                 y))
...
class Person:
    ...
    def check(self):
        ...
        if (keys[pygame.K_s] or keys[pygame.K_DOWN]) \
                and self.rect.y < self.height - self.rect.height:
            self.rect.move_ip(0, self.speed)
        ...
        if (keys[pygame.K_d] or keys[pygame.K_RIGHT]) \
                and self.rect.x < self.width - self.rect.width:
            self.rect.move_ip(self.speed + 2, 0)

        if keys[pygame.K_SPACE] and \
                pygame.time.get_ticks() > self.rocket_timer + self.rocket_time:
            self.rocket_timer = pygame.time.get_ticks()
            self.rocket_spawn_function()

        if not self.state and \
